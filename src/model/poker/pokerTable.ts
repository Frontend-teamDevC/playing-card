...
export default class pokerTable extends Table {
  ...
  evaluateAndGetRoundResults(): string {
    const hashMap: Map<PokerHandType, number> = new Map<PokerHandType, number>()
    ...
    if (winnerPlayer.length > 1) {
      ...
      // フルハウス or スリーカード
      else if (strongestHand == 'full house' || strongestHand == 'three card') {
        ...
        // 同じだったら => full houseならpairs of two　で確認 else 引き分け
          ...
          if (strongestHand == 'three card') {
            winnerPlayer = winnerPlayer.filter(
              (player) => player.pairsOfThreeList[0] == maxThreeCard
            )
            this.drawSplitChip(winnerPlayer)
          } else {
            if (winnerPlayer[0].pairsOfTwoList[0] != maxTwoCard) {
              ...
              winnerPlayer = winnerPlayer.filter(
                (player) => player.pairsOfTwoList[0] == maxTwoCard
              )
              this.drawSplitChip(winnerPlayer)
            }
          }
      ...
      //　ツーペア ワンペア比較
      else if (strongestHand == 'two pair' || strongestHand == 'one pair') {
  ...
  drawSplitChip(winnerPlayers: pokerPlayer[]): void {
    winnerPlayers.map(
      (player) => (player.chips += Math.floor(this.pot / winnerPlayers.length))
    )
  }
  ...
  getResultLog(): string {
    ...
    for (let i = 0; i < this.players.length; i++) {
      roundLog +=
        this.players[i].chips + (i != this.players.length - 1 ? ',' : '')
    }
    return roundLog
  }
    ...
    if (player.type == 'dealer') {
      ...
      switch (gameDecision.action) {
        ...
        case 'blind':
          ...
          player.bet = this.blindCounter == 0 ? this.smallBlind : this.bigBlind
          this.blindCounter++
  ...
  haveTurn(userData?: PokerActionType): void {
    ...
    if (this.allPlayerActionResolved()) {
      ...
      } else if (player.gameStatus == 'fold' || player.gameStatus == 'allin') {
  ...
  /**
   * 全プレイヤーがfoldかallinかどうかを返す関数
   *
   * @returns {boolean} - 全プレイヤーがfoldかallinかどうか
   */
  ...
  /**
   * プレイヤーの所持金が0の場合、gameStatusをfoldに変更する関数
   *
   * @returns {void}
   */
  changeFoldwithnomoney(): void {
    this.players.map((player) => {
  ...
  /**
   * 所持チップが0でないプレイヤーが１人だけかどうかを返す関数
   *
   * @returns {boolean} - 所持チップが0でないプレイヤーが１人だけかどうか
   */
  ...
  /**
   * 現在のプレイヤーの一つ前のプレイヤーを返す関数
   *
   * @returns {pokerPlayer} - 現在のプレイヤーの一つ前のプレイヤー
   */
  getoneBeforePlayer(): pokerPlayer {
    return this.players[
      (this.playerIndexCounter + this.players.length - 1) % this.players.length
    ]
  }
}
